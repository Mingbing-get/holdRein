...
export interface GomokuTaskGameStorage {
  readonly deleteGame: (taskId: string) => Promise<void>;
  readonly readGame: (taskId: string) => Promise<PersistedGomokuTaskGame | null>;
  readonly writeGame: (game: PersistedGomokuTaskGame) => Promise<void>;
}

export interface CreateGomokuTaskGameStorageOptions {
  readonly storageRoot?: string;
}

export function createGomokuTaskGameStorage(
  options: CreateGomokuTaskGameStorageOptions = {}
): GomokuTaskGameStorage {
  ...
  return {
    ...
    async readGame(taskId) {
      try {
        ...
        return JSON.parse(raw) as PersistedGomokuTaskGame;
      } catch (error) {
...
function defaultStorageRoot(): string {
  return join(homedir(), ".hold-rein", "plugin-data", "gomoku");
}
